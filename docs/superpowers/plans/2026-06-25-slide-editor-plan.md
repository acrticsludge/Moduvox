...
**Goal:** Add slide editor mode that replaces the upload zone after a PPTX is selected, with slide preview, narration textarea, generate button, and audio player.

**Architecture:** Single new `SlideEditor` component managing all editor state (mock slides, current index, audio visibility). Modify `PptxUploadZone` to accept an `onFileAccepted` callback. Modify the page to switch between upload and editor modes.

**Spec:** `docs/superpowers/specs/2026-06-25-slide-editor-design.md`
...
|------|--------|---------------|
| `components/dashboard/SlideEditor.tsx` | Create | Slide editor with preview, textarea, generate btn, audio player, nav arrows |
| `components/dashboard/PptxUploadZone.tsx` | Modify | Add `onFileAccepted` prop, call it after validation |
| `app/dashboard/projects/[id]/presentations/[presentationId]/page.tsx` | Modify | Add mode state, switch between upload and editor |
| `docs/superpowers/specs/2026-06-25-slide-editor-design.md` | Create | Already written |

---

### Task 1: Create SlideEditor component

- [ ] **Step 1: Create the component**

Create `frontend/components/dashboard/SlideEditor.tsx`:
...
export function SlideEditor() {
  ...
  function handleGenerate() {
    setGenerating(true)
    // Simulate generation delay, then reveal audio
    setTimeout(() => {
      setSlides((prev) =>
        prev.map((s) =>
...
git commit -m "feat: add SlideEditor with preview, narration, generate, and audio player"
```

---

### Task 2: Modify PptxUploadZone to accept onFileAccepted callback

- [ ] **Step 1: Add the prop and call it**

Edit `frontend/components/dashboard/PptxUploadZone.tsx`:
...
Inside `validateAndSet`, add the callback call after setting the file:
```tsx
...
git commit -m "feat: add onFileAccepted callback to PptxUploadZone"
```

---

### Task 3: Modify page to switch modes

- [ ] **Step 1: Update the page component**

Edit `frontend/app/dashboard/projects/[id]/presentations/[presentationId]/page.tsx`:

Add the import for SlideEditor:
```tsx
import { SlideEditor } from "@/components/dashboard/SlideEditor"
```

Add mode state after the existing state declarations:
```tsx
const [mode, setMode] = useState<"upload" | "editor">("upload")
```

Replace the content section where `PptxUploadZone` is rendered — instead of always rendering it, conditionally render based on mode:
```tsx
...
git commit -m "feat: add mode switching from upload to editor"
```
...
1. **Spec coverage:**
   - Slide editor replaces upload zone after file accepted ✅
   - Slide preview with number ✅
   - Navigation arrows with "Slide X of Y" ✅
   - Narration textarea (editable, per-slide) ✅
   - Generate button (disables after clicked, shows "Generated") ✅
   - Audio player appears after generate with smooth animation ✅
   - 3 mock slides ✅
   - Sidebar unchanged ✅

2. **Placeholder scan:** No TBDs or TODOs.

3. **Type consistency:** `SlideData` type used consistently. `onFileAccepted` matches between component and page usage.
