...
const [expireDate, setExpireDate] = useState<Date | undefined>(undefined)
  ...
  const isPublic = !settings.email_gate_enabled && !settings.has_password
  ...
  return (
    <div className={`space-y-5 rounded-xl border bg-white p-6 transition-all duration-300 ${
      ...
      {showPublicConfirm && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[#18181B]/40 p-4"
          onClick={(e) => { if (e.target === e.currentTarget) setShowPublicConfirm(false) }}
        >
          <div className="w-full max-w-md rounded-xl border border-zinc-200 bg-white p-6 shadow-xl">
            <h2 className="text-base font-semibold text-[#18181B]">Remove password protection?</h2>
            <p className="mt-2 text-sm text-[#71717A]">This will remove password protection. Anyone with the link can view.</p>
            <div className="mt-6 flex justify-end gap-3">
              <button
                type="button"
                onClick={() => setShowPublicConfirm(false)}
                className="rounded-lg border border-zinc-200 px-4 py-2.5 text-sm font-medium text-zinc-600 hover:text-[#18181B]"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-lg bg-[#18181B] px-4 py-2.5 text-sm font-medium text-white hover:bg-[#27272A]"
              >
                Remove Password
              </button>
            </div>
