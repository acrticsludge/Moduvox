...
export default function ViewPresentationPage() {
  ...
  async function prefetchAllSlideBlobs(slideList: { slideNumber: number; pdfUrl: string | null }[]) {
    ...
    // Override react-pdf's default workerSrc ('pdf.worker.mjs' — a bare module
    // specifier that browsers cannot resolve). Must happen unconditionally
    // because react-pdf already sets a truthy default at import time, so a
    // guard like if (!workerSrc) would never fire.
                        ...
                        <div className="absolute left-0 right-0 top-0 flex items-center justify-between p-4">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className={`${fullscreenControlsVisible ? "pointer-events-auto" : "group-hover:pointer-events-auto"} flex h-9 w-9 items-center justify-center rounded-full bg-black/50 text-white/70 backdrop-blur-sm transition-colors hover:bg-black/70`}
